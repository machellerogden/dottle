...
const emitLabeledEdge = (a, b, label) => `"${a}" -> "${b}" [label="${label}" fontcolor="gray" fontsize=9 fontname="Helvetica"];`;
...
function emit(dot) {
    ...
    if (dot.type === 'edge') return dot.label
        ? emitLabeledEdge(...dot.points, dot.label)
        : emitEdge(...dot.points);
